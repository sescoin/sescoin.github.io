-- ============================================================
-- Retablit les actions de pret absentes de schema.sql
--
-- reject_loan / cancel_loan / delete_loan n'ont jamais existe dans le schema
-- de base : elles ont ete introduites par la migration 20260521 (delete_loan
-- ayant ete revue par la 20260702). Apres une reinitialisation depuis
-- schema.sql, annuler une demande echoue avec :
--   « Could not find the function public.cancel_loan(...) in the schema cache »
-- ============================================================
...
declare
  v_loan public.loans;
begin
  if auth.uid() is distinct from p_lender_id
...
declare
  v_loan public.loans;
begin
  if auth.uid() is distinct from p_borrower_id
     and not public.current_profile_is_admin() then
    raise exception 'Accès refusé.';
  end if;
...
-- Version de la 20260702 : autorise aussi la suppression d'une demande en
-- attente dont l'echeance est passee.
